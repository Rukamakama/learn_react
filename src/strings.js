const POST_TEXT = 'Lorem ipsum, or lipsum as it is sometimes known, ' +
    'is dummy text used in laying out print, graphic or web designs. ' +
    'The passage is attributed to an unknown typesetter in the 15th ' +
    'century who is thought to have scrambled parts of Cicero\'s De Finibus' +
    ' Bonorum et Malorum for use in a type specimen book.';
const SLOGAN = 'With big goals, little fry catch serve as bait !';
const CREAT_POST = 'Create a new post by providing below information';
const POST_TITLE = 'Optimal code require more to build and less to maintain';
const BLOG_TITLE = 'Hi! This is my main blog where I publish every time';
const POST_OWNER = 'Rukamakama';

const STR_ABOUT = 'About';
const STR_POST = 'Post';
const STR_DESC = 'DESCRIPTION';
const STR_ADD_POST = 'Add a new post'
const STR_TITLE = 'TITLE';
const STR_POST_PREV = 'Post preview';
const STR_LOGIN = 'Login';
const STR_USERNAME = 'Username';
const STR_PASSWORD = 'Password';

export {
    POST_TEXT, SLOGAN, CREAT_POST, POST_TITLE, BLOG_TITLE, POST_OWNER,
    STR_ABOUT, STR_POST, STR_ADD_POST, STR_DESC, STR_LOGIN, STR_PASSWORD,
    STR_POST_PREV, STR_TITLE, STR_USERNAME
}